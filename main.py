...
class XmindToXsl(XlwtSeting):
    ...
    def write_excel(self, result='', performer='', editionname=''):
        ...
        row0 = ["testCaseId", '模块', '子模块', '测试点', '前置条件', '操作步骤', '预期结果',
                '实际结果']  # 写成excel表格用例的要素
        for i in range(len(row0)):
            self.worksheet.write(0, i, row0[i])
        x = 0  # 写入数据的当前行数
        style = self.template_two()
        for a in range(len(self.xm['topics'])):  # 第一级 枪械
            main_class = self.xm['topics'][a]
            for b in range(len(main_class['topics'])):  # 第二级 枪械》瞄准
                secondary_class = main_class['topics'][b]
                for c in range(len(secondary_class['topics'])):  # 第三级 枪械》瞄准》测试点（键位操作）
                    test_point = secondary_class['topics'][c]
                    for d in range(len(test_point['topics'])):  # 第四级 键位操作》前置条件
                        preconditions = test_point['topics'][d]
                        for e in range(len(preconditions['topics'])):  # 第五级  操作步骤
                            operate_step = preconditions['topics'][e]
                            test_expect = operate_step['topics'][0]['title']  # 预期结果
                            x += 1
                            # worksheet.write()
                            self.heights(self.worksheet, x, size=2)
                            self.worksheet.write(x, 1, main_class['title'], style)
                            self.worksheet.write(x, 2, secondary_class['title'], style)
                            self.worksheet.write(x, 3, test_point['title'], style)
                            self.worksheet.write(x, 4, preconditions['title'], style)
                            self.worksheet.write(x, 5, operate_step['title'], style)  # 写入操作步骤
                            self.worksheet.write(x, 6, test_expect, style)  # 写入预期结果
...
class MainUI(object):
    ...
    def create_widgets(self):
        ...
        f_btn.place(x=180, y=355, width=200, height=45)  # 设置框架的大小，及在top窗口显示位置
        submit_button = tkinter.Button(f_btn, text="提交", command=self.get_value, width=28, height=2, bg="#00FFFF")  # 设置按钮的文字，调用方法，大小，颜色，显示框架
